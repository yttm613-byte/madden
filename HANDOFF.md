...
| Broadcast graphics (`banner`, `driveLine`, `halftimeCard`) | 3270 | `banner(title, sub, team)` reuses the possession plate for scores: TOUCHDOWN / PICK SIX / KICK RETURN TD with the drive (`G.drive` counts plays from `startPossession`, yards and game time) and FIELD GOAL with its drive. Halftime runs the open's flyover with the score and first-half numbers (`G._halftime` in the `intro` phase), and clears any menu the half interrupted. |
