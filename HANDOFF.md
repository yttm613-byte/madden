...
| Broadcast graphics (`banner`, `driveLine`, `halftimeCard`, `lowerThird`) | 3270 | `banner(title, sub, team)` reuses the possession plate for scores: TOUCHDOWN / PICK SIX / KICK RETURN TD with the drive (`G.drive` counts plays from `startPossession`, yards and game time) and FIELD GOAL with its drive. Halftime runs the open's flyover with the score and first-half numbers (`G._halftime` in the `intro` phase), and clears any menu the half interrupted. `lowerThird(team, n, kind)` is the TV name-and-stats strip in the top-left after a 12+ yard run, a 15+ yard catch, a sack or a touchdown (after its banner), from `recordPlay`; not on back-to-back plays otherwise (`_l3Since`). |
| Video boards (`buildVideoBoards`, `drawVideoBoard`) | 4340 | One 1024×300 canvas on both end-zone boards (two merged meshes: frames and legs, screens). The boards stand behind the end-zone seats with their foot just over the back row, so the chase camera, which always looks up the field, sees the far one at the top of the shot until the offense is inside the 20; the open's flyover frames it too. `drawVideoBoard` runs from `render3D` and redraws only when its key changes (scores, quarter, whole seconds, down, abbreviations, kits, final), about once a second; TOUCHDOWN in the scorer's jersey colour while `G.phase==='dead' && G._td`. The screen material is unfogged on a clear night (the far board is 140 yards off and fog would wash it out) and fogged in rain and snow (`applyWeather`). Text is drawn with a max width so a fallback font never overflows. |
| Coach tips (`tip`, `TIPS`) | 560 | The first kick return, play call, run, pass, coverage call, defensive snap and kick each show a short tip once per device (localStorage `gb_tips`), worded for keyboard, controller or touch. Hooks: `enterPlayCall`, `choosePlay`, `showDefCall`, `startKick`, and the `update` frame for kick returns and the first defensive snap. Off in two-player games and in harness builds (`__gbNoTips`). |
